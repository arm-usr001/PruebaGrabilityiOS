    ...
    public func setProgress(to progressConstant: Double, withAnimation: Bool) {
        ...
        let timer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { (timer) in
            if currentTime >= animationDuration {
                ...
                self.label.text = "\(Double(1 * percent))"
                self.label.text = "\(((Double(exactly: 1 * percent) ?? 0.0) / 10).fractionDigitsRounded(to: 1))"
                self.configLabel()
            }
        }
        timer.fire()
...
extension FloatingPoint {
    func fractionDigitsRounded(to digits: Int, roundingMode:  NumberFormatter.RoundingMode = .halfEven) -> Double {
        Formatter.number.roundingMode = roundingMode
        Formatter.number.minimumFractionDigits = digits
        Formatter.number.maximumFractionDigits = digits
        return ((Formatter.number.string(for:  self) ?? "0.0") as NSString).doubleValue
    }
}
